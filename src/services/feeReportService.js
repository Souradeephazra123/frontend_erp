...
class FeeReportService {
  ...
  static getMockClassReport(params) {
    const mockStudents = [
      { id: 'STU001', name: 'John Doe', class: params.class || '10th Grade', section: params.section || 'A' },
      { id: 'STU002', name: 'Jane Smith', class: params.class || '10th Grade', section: params.section || 'A' },
      { id: 'STU003', name: 'Mike Johnson', class: params.class || '10th Grade', section: params.section || 'A' },
      { id: 'STU004', name: 'Sarah Wilson', class: params.class || '10th Grade', section: params.section || 'A' },
      { id: 'STU005', name: 'David Brown', class: params.class || '10th Grade', section: params.section || 'A' }
    ];
    ...
    mockStudents.forEach((student, index) => {
      ...
      mockData.summary.totalDue += (tuitionFee - tuitionPaid) + (libraryFee - libraryPaid) + (busFee - busPaid);
    });
  ...
  static async getClasses() {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_API_URL}/api/classes`);
      return response.data;
    } catch (error) {
      console.warn('Backend endpoint not available, using mock classes:', error.message);
      return [
        { id: 1, name: '1st Grade' },
        { id: 2, name: '2nd Grade' },
        { id: 3, name: '3rd Grade' },
  ...
  static async getSections(classId) {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_API_URL}/api/sections/${classId}`);
      return response.data;
    } catch (error) {
      console.warn('Backend endpoint not available, using mock sections:', error.message);
      return [
        { id: 1, name: 'A' },
        { id: 2, name: 'B' },
        { id: 3, name: 'C' }
  ...
  static async getStudents() {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_API_URL}/api/students`);
      return response.data;
    } catch (error) {
      console.warn('Backend endpoint not available, using mock students:', error.message);
      return [
        { id: 'STU001', name: 'John Doe', class: '10th Grade', studentId: 'STU001' },
        { id: 'STU002', name: 'Jane Smith', class: '10th Grade', studentId: 'STU002' },
        { id: 'STU003', name: 'Mike Johnson', class: '9th Grade', studentId: 'STU003' },
        { id: 'STU004', name: 'Sarah Wilson', class: '9th Grade', studentId: 'STU004' },
        { id: 'STU005', name: 'David Brown', class: '8th Grade', studentId: 'STU005' },
        { id: 'STU006', name: 'Emily Davis', class: '8th Grade', studentId: 'STU006' },
        { id: 'STU007', name: 'Chris Miller', class: '7th Grade', studentId: 'STU007' },
        { id: 'STU008', name: 'Lisa Anderson', class: '7th Grade', studentId: 'STU008' }
      ];
    }
  }
}
